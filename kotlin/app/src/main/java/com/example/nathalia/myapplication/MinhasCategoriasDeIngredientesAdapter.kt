...
class MinhasCategoriasDeIngredientesAdapter( var categoriasList: MutableList<Item>,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        when(viewType){
            CATEGORIA -> { view = inflater.inflate(R.layout.list_layout_categorias, parent,false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if ( holder.itemViewType == CATEGORIA ) {
            ...
            if (itensDoHolder != null) {
                ...
                itensDoHolder.checkBox.setOnClickListener { view ->
                    run {
                        if ((view as CheckBox).isChecked)
                            if (!listaingredientes.contains(item.text)) {
                                listaingredientes.add(item.text)
                            }
                        else listaingredientes.remove(item.text)
